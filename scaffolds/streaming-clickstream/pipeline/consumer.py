...
dicts, and returns them as a list.
"""
...
def run_consumer(group_id: str, max_records: int) -> list[dict]:
    ...
    Behavior:
        ...
        * Default auto-commit is fine for this stage.
        * Deserialize each record's value (bytes) as JSON.
        * Return the list of decoded events in the order they were consumed.

    Manual offset commit and at-least-once delivery are deliberately out of
    scope here — Session 2 covers them alongside Spark checkpointing.
