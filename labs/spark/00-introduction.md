...
├── pipeline/                         ← YOU MODIFY
│   ├── etl.py                        ← run_etl
│   └── serving.py                    ← total_revenue, avg_revenue
├── jobs/run_etl_job.py               ← spark-submit entrypoint (provided)
├── pipeline/scheduler.py             ← the minute-cadence scheduler (provided)
├── tests/                            ← stage tests (provided, don't edit)
└── producer/producer.sh              ← the file-dropping producer (provided)
```

You modify exactly two files: `pipeline/etl.py` and `pipeline/serving.py`.
